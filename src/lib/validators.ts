...
export const creatorSignupBody = z.object({
  ...
  })).min(1, 'At least one social account is required').max(4, 'Maximum 4 social accounts'),
...
const CRYPTO_NETWORKS = ['BTC', 'ETH', 'USDC_ERC20', 'USDC_TRC20', 'USDT_ERC20', 'USDT_TRC20', 'SOL'] as const;
...
export const addSocialBody = z.object({
  platform: z.enum(['instagram', 'tiktok', 'youtube', 'snapchat', 'x', 'facebook']),
  handle: z.string().trim().min(1).max(80),
