...
interface CarouselVideo { id: string; url: string; title: string | null }

export function CreatorHomepage() {
  const [videos, setVideos] = React.useState<CarouselVideo[]>([]);

  React.useEffect(() => {
    fetch('/api/public/homepage-videos')
      .then((r) => r.json())
      .then((d) => { if (Array.isArray(d.videos)) setVideos(d.videos); })
      .catch(() => {});
  }, []);
        ...
        <div className="container max-w-6xl px-4">
          <FadeIn>
            <div className="text-center max-w-2xl mx-auto mb-10">
              <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight">Campaign spotlight</h2>
              <p className="mt-3 text-muted-foreground">
                See the kind of content our creators are producing right now.
              </p>
            </div>
          </FadeIn>
          <HomepageCarousel videos={videos} />
        </div>
      </section>
