...
import {
  ArrowRight, BadgeCheck, Banknote, ChevronDown, Eye, Landmark, ShieldCheck,
  Sparkles, TrendingUp, Users, Wallet,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { MarketingNav } from '@/components/marketing/marketing-nav';
import { MarketingFooter } from '@/components/marketing/marketing-footer';
import { PayoutWall, type PayoutWallCard } from '@/components/marketing/payout-wall';
...
export function CreatorHomepage({ payoutCards = [] }: { payoutCards?: PayoutWallCard[] }) {
  return (
        ...
        <div className="container max-w-7xl px-4 pt-14 pb-20 sm:pt-20 sm:pb-28 grid lg:grid-cols-[1.15fr_1fr] gap-14 items-center">
          <FadeIn>
            ...
            <div className="mt-9 flex flex-col sm:flex-row gap-3">
              ...
              <Button size="xl" variant="outline" className="border-transparent shadow-sm" asChild>
            ...
            <div className="mt-10 flex items-center gap-2.5 text-muted-foreground text-sm font-medium">
              ...
              <div className="flex items-center gap-2.5">
                {PLATFORMS.map((p) => (
                  <PlatformIcon key={p} platform={p} className="h-[18px] w-[18px]" />
                ))}
              </div>
            </div>
          </FadeIn>
          ...
          <div className="relative h-[440px] hidden lg:block" aria-hidden>
            <FloatCard rotation="-8deg" delay="0s" className="absolute left-0 top-14 w-60 z-10">
            ...
            <FloatCard rotation="3deg" delay="0.9s" className="absolute right-24 top-0 w-64 z-20">
            ...
            <FloatCard rotation="8deg" delay="1.8s" className="absolute right-0 bottom-14 w-60 z-30">
              <div className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Available to pay out</div>
              <div className="mt-1 text-3xl font-extrabold tabular-nums tracking-tight">$2,140.00</div>
      ...
      <section id="how-it-works" className="py-24">
        <div className="container max-w-6xl px-4">
          <SectionHeader
            ...
            title={<>From posting to payout<br className="hidden sm:block" /> in 3 simple steps</>}
            ...
            {[
              ...
              <FadeIn key={s.title} delay={i * 100}>
                <div className="h-full rounded-3xl bg-card border border-black/[0.06] shadow-sm p-6 flex flex-col">
                  ...
                  <div className="text-xs font-bold text-primary uppercase tracking-wide">{s.step}</div>
                  <div className="mt-1.5 text-xl font-extrabold tracking-tight">{s.title}</div>
                  <p className="mt-2 text-sm text-muted-foreground font-medium leading-relaxed">{s.body}</p>
                </div>
              </FadeIn>
            ))}
          </div>
              ...
              <div className="mt-6 rounded-2xl bg-muted/60 p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground font-medium">Active on all campaigns</div>
                  </div>
                </div>
                <span className="rounded-full bg-green-500/10 text-green-600 text-[11px] font-bold px-3 py-1">ACTIVE</span>
              </div>
            </BentoCard>
            <BentoCard
            ...
            >
              <div className="mt-6 space-y-2">
                {[
                  ...
                  <div key={t} className="rounded-xl bg-muted/60 px-3.5 py-2.5 flex items-center justify-between text-xs font-semibold">
                ...
                <div className="flex -space-x-2">
                  {[0, 1, 2, 3].map((i) => (
                    <span
                      ...
                      style={{ background: `hsl(${20 + i * 12} 90% ${50 + i * 4}%)` }}
                    >
                      {['+', '✦', '+', '✦'][i]}
                    </span>
                  ))}
            ...
            >
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-muted/60 p-4">
                  ...
                  <div className="text-xs text-muted-foreground font-medium">Worldwide</div>
                </div>
                <div className="rounded-2xl bg-muted/60 p-4">
                  <Wallet className="h-5 w-5 text-primary mb-2" />
                  <div className="text-sm font-bold">Crypto</div>
                  <div className="text-xs text-muted-foreground font-medium">USDC &amp; more</div>
                </div>
              </div>
            </BentoCard>
          </div>
                ...
                <div className="h-full rounded-3xl bg-card border border-black/[0.06] shadow-sm p-7 flex flex-col">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground font-medium">{t.handle}</div>
                    </div>
                  </div>
                  <p className="text-[15px] font-semibold leading-relaxed tracking-tight flex-1">{t.quote}</p>
                  <div className="mt-6 pt-5 border-t">
                    <div className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Revenue generated</div>
                    <div className="mt-1 text-sm font-extrabold text-primary">{t.stat}</div>
                  </div>
                </div>
              </FadeIn>
            ))}
      ...
      <section className="py-24">
        <div className="container max-w-3xl px-4">
          ...
          <div className="mt-12 rounded-3xl bg-card border border-black/[0.06] shadow-sm p-8 sm:p-10 text-center">
            ...
            <h3 className="text-display text-2xl sm:text-3xl">{MARKETING.creatorCta.heading}</h3>
            <p className="mt-2 text-muted-foreground font-medium">{MARKETING.creatorCta.sub}</p>
...
function SectionHeader({
  ...
  return (
    <div className="text-center max-w-2xl mx-auto mb-14">
      ...
      <p className="mt-4 text-muted-foreground font-medium leading-relaxed">{sub}</p>
    </div>
  );
}
  ...
  const chunk = (
    <>
      {Array.from({ length: 8 }).map((_, i) => (
        <span key={i} className="mx-6 inline-flex items-center gap-3 text-sm font-bold uppercase tracking-wide whitespace-nowrap">
          Trusted by {MARKETING.stats[0].value} creators <span className="text-base leading-none">✦</span>
        </span>
      ))}
    </>
  );
  return (
    <div className="relative overflow-hidden bg-primary text-white py-3 -rotate-1 scale-[1.02]" aria-hidden>
...
function FloatCard({
  rotation, delay, className, children,
}: { rotation: string; delay: string; className?: string; children: React.ReactNode }) {
  return (
    <div
  ...
  return (
    <div className="w-full max-w-[210px] rounded-2xl bg-card shadow-md p-4">
      <div className="flex items-center gap-2.5 mb-3">
        <span className="grid h-8 w-8 place-items-center rounded-full bg-primary text-white text-xs font-bold">Y</span>
        <div>
          <div className="text-xs font-bold">You</div>
          <div className="text-[10px] text-muted-foreground font-medium">@yourhandle</div>
        </div>
  ...
  return (
    <div className="w-full max-w-[210px] rounded-2xl bg-card shadow-md p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="rounded-full bg-accent text-primary text-[9px] font-bold px-2 py-0.5 uppercase">Clipping</span>
      ...
      <div className="mt-1.5 text-[10px] font-semibold text-muted-foreground">$21,360 of $30,000 paid out</div>
...
function MockPayout() {
  return (
    <div className="w-full max-w-[210px] rounded-2xl bg-card shadow-md p-4">
      ...
      <div className="mt-0.5 text-2xl font-extrabold tabular-nums tracking-tight">$1,862.50</div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <div className="rounded-full bg-primary text-white text-center text-[11px] font-bold py-2">Bank</div>
        <div className="rounded-full bg-muted text-center text-[11px] font-bold py-2">Crypto</div>
      </div>
    </div>
  );
}
...
function BentoCard({
  ...
  return (
    <div className={`rounded-3xl bg-card border border-black/[0.06] shadow-sm p-7 flex flex-col ${className ?? ''}`}>
      ...
      <p className="mt-2 text-sm text-muted-foreground font-medium leading-relaxed">{body}</p>
      {children}
    </div>
  );
}

function CampaignCard({ c }: { c: (typeof MARKETING.showcaseCampaigns)[number] }) {
  ...
  return (
    <div className="h-full rounded-3xl bg-background border border-black/[0.06] shadow-sm p-6 flex flex-col">
      <div className="flex items-center gap-2 text-[11px] font-bold">
        <span className="rounded-full bg-accent text-primary px-2.5 py-1 uppercase tracking-wide">{c.category}</span>
        <span className="rounded-full bg-muted px-2.5 py-1 uppercase tracking-wide text-muted-foreground">{c.niche}</span>
        <span className="ml-auto font-semibold text-muted-foreground">{c.age}</span>
      </div>
      <div className="mt-4 text-lg font-extrabold tracking-tight leading-snug">{c.title}</div>
      <div className="mt-1 text-sm font-semibold text-muted-foreground">{c.brand}</div>
      <div className="mt-5">
        <div className="h-2 rounded-full bg-muted overflow-hidden">
          <div className="h-full rounded-full bg-primary" style={{ width: `${pct}%` }} />
        </div>
        <div className="mt-2 flex justify-between text-xs font-bold">
          <span>{money(c.paid)} <span className="text-muted-foreground font-semibold">/ {money(c.budget)}</span></span>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="rounded-2xl bg-card border border-black/[0.06] shadow-sm overflow-hidden">
      ...
      {open && (
        <p className="px-6 pb-5 text-sm text-muted-foreground font-medium leading-relaxed">{a}</p>
      )}
    </div>
  );
}
