...
export default function AdminRepostPayoutsPage() {
  ...
  React.useEffect(() => {
    const t = setTimeout(() => {
      ...
      u.set('page', '1');
      router.replace(`/admin/reposting/payouts?${u.toString()}`, { scroll: false });
    }, 300);
  ...
  const list = useQuery<ListResp>({
    queryKey: ['admin', 'repost-payouts', search, status, page],
    queryFn: () => {
      const u = new URLSearchParams({ page: String(page), pageSize: '25', status });
      if (search) u.set('search', search);
      return api.get<ListResp>(`/api/admin/repost/payouts?${u.toString()}`);
    },
    staleTime: 0,
  ...
  const approveMut = useMutation({
    mutationFn: ({ id, amountPaid, adminNote }: { id: string; amountPaid: number; adminNote?: string }) =>
      fetch(`/api/admin/repost/payouts/${id}`, {
        method: 'PATCH',
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['admin', 'repost-payouts'] });
    },
  ...
  return (
    <>
      <PageHeader
        title="Repost Payouts"
        description="Separate from the main Payouts queue — these draw only from each creator's repost wallet."
        actions={
      ...
      <div className="container max-w-6xl py-6">
        <Card>
          {list.isLoading ? (
            ...
            <EmptyState icon={Wallet} title="No repost payout requests" description={search ? 'Try a different search term.' : 'When creators request repost payouts, they show up here.'} />
          ) : (
            <ul className="divide-y">
              {list.data!.entries.map((p) => (
                <PayoutRow
                  key={p.id}
                  entry={p}
                  onApprove={() => setApproveModal({ open: true, id: p.id, amountAtRequest: parseFloat(p.amountAtRequest), amountPaid: p.amountAtRequest, adminNote: '' })}
                  onReject={() => setRejectModal({ open: true, id: p.id, adminNote: '' })}
                  onCancel={() => { if (confirm('Cancel this payout request?')) cancelMut.mutate(p.id); }}
      ...
      {approveModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <Card className="w-full max-w-md p-6 space-y-5 shadow-2xl">
            <div>
              <h2 className="font-semibold text-base flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-green-600" /> Approve repost payout</h2>
              <p className="text-xs text-muted-foreground mt-1">Creator requested {formatMoney(approveModal.amountAtRequest)}. Enter the amount you actually paid.</p>
            </div>
                ...
                if (Number.isFinite(paid) && paid < req) {
                  ...
                  return (
                    <p className="text-xs text-orange-600 dark:text-orange-400 flex items-center gap-1">
                      <AlertCircle className="h-3 w-3" /> Penalty of {formatMoney(penalty)} will be recorded (requested − paid).
                    </p>
                  );
                }
                return null;
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setApproveModal({ open: false })} disabled={approveMut.isPending}>Cancel</Button>
              <Button
                onClick={() => {
      ...
      {rejectModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <Card className="w-full max-w-md p-6 space-y-5 shadow-2xl">
            <div>
              <h2 className="font-semibold text-base flex items-center gap-2"><XCircle className="h-4 w-4 text-destructive" /> Reject repost payout</h2>
              <p className="text-xs text-muted-foreground mt-1">Provide a reason for rejection — the creator will see this.</p>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setRejectModal({ open: false })} disabled={rejectMut.isPending}>Cancel</Button>
              <Button
                variant="destructive"
                onClick={() => {
  ...
  return (
    <li className="p-4">
      <div className="flex items-start gap-3">
        <button onClick={() => setOpen((v) => !v)} className="mt-0.5 text-muted-foreground hover:text-foreground" aria-label={open ? 'Collapse' : 'Expand'}>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {entry.employee.firstName && (
              <span className="text-xs text-muted-foreground">({entry.employee.firstName}{entry.employee.lastName ? ' ' + entry.employee.lastName : ''})</span>
            )}
          ...
          <div className="text-xs text-muted-foreground mt-1 flex items-center gap-2 flex-wrap">
            ...
            {amountPaid !== null && (<><span>·</span><span className="text-green-600 dark:text-green-400 font-medium">{formatMoney(amountPaid)} paid</span></>)}
            {penalty !== null && penalty > 0 && (<><span>·</span><span className="text-orange-600 dark:text-orange-400">{formatMoney(penalty)} penalty</span></>)}
            <span>·</span>
            <span>requested {formatRelative(entry.createdAt)}</span>
          </div>

          {entry.adminNote && <p className="text-xs text-muted-foreground mt-1 italic">Admin note: {entry.adminNote}</p>}

          {open && (
            <div className="mt-3 rounded-md border bg-muted/20 p-3 space-y-2">
              {Object.entries(entry.details).map(([k, v]) => <DetailRow key={k} label={prettyLabel(k)} value={v} />)}
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {(entry.status === 'REQUESTED' || entry.status === 'IN_PROGRESS') && (
            <>
              <Button size="sm" onClick={onApprove} className="bg-green-600 hover:bg-green-700 text-white"><CheckCircle2 className="h-3.5 w-3.5" /> Approve</Button>
              <Button size="sm" variant="destructive" onClick={onReject}><XCircle className="h-3.5 w-3.5" /> Reject</Button>
...
function prettyLabel(k: string): string {
  const map: Record<string, string> = {
    holderName: 'Holder name', bankName: 'Bank', iban: 'IBAN', swift: 'SWIFT', network: 'Network', address: 'Wallet',
  };
  return map[k] ?? k;
}
